...
<header class="sticky top-0 z-50 border-b border-edge bg-base/80 backdrop-blur-md">
    <div class="container-page flex h-16 items-center justify-between">
        ...
        <nav id="nav-menu" class="max-lg:absolute max-lg:top-full max-lg:left-0 max-lg:right-0 max-lg:flex-col max-lg:gap-1 max-lg:border-b max-lg:border-edge max-lg:bg-base max-lg:p-4 lg:flex lg:items-center lg:gap-8">
            <a href="{{ route('home') }}" class="{{ request()->routeIs('home') ? 'text-ink' : 'text-muted hover:text-ink' }} text-sm transition-colors">Home</a>
            <a href="{{ route('product') }}" class="{{ request()->routeIs('product') ? 'text-ink' : 'text-muted hover:text-ink' }} text-sm transition-colors">Product</a>
            <a href="{{ route('pricing') }}" class="{{ request()->routeIs('pricing') ? 'text-ink' : 'text-muted hover:text-ink' }} text-sm transition-colors">Pricing</a>
            <a href="{{ route('customers') }}" class="{{ request()->routeIs('customers') ? 'text-ink' : 'text-muted hover:text-ink' }} text-sm transition-colors">Customers</a>
            <a href="{{ route('docs') }}" class="{{ request()->routeIs('docs') ? 'text-ink' : 'text-muted hover:text-ink' }} text-sm transition-colors">Docs</a>
            <a href="{{ route('pricing') }}" class="rounded-full bg-coral px-4 py-2 text-sm font-semibold text-[#0a0a0b] transition-transform hover:-translate-y-px max-lg:mt-2 lg:ml-2">Start free</a>
        </nav>

        <div class="flex items-center gap-2">
            <button id="theme-toggle" type="button" aria-label="Toggle color theme" class="grid h-9 w-9 place-items-center rounded-full border border-edge text-muted transition-colors hover:text-ink">
